...
E preencho o formulario "${NOME}", "${EMAIL}" e "${SENHA}"
    ...
    Wait Until Element Is Visible       ${E_MAIL}    # aguardando o campo de input "e-mail" ficar visível
    Input Text                          ${E_MAIL}            ${EMAIL}
...
Quando efetuar o login com "${EMAIL}" e "${SENHA}"
    Click Element                       ${BTN_LOGIN}   # clicando no bnt LOGIN através do texto
    Wait Until Element Is Enabled       ${E_MAIL}
    Input Text                          ${E_MAIL}            ${EMAIL}
    Input Password                      ${PASSWORD}         ${SENHA}
...
Então visualizar os componentes do formulário do perfil
    ...
    Should Not Be Equal         xpath=(//span[contains(text(),'Cypress Test e2e')])[2]
    Should Not Be Equal         xpath=//span[normalize-space()='WCSA Soluções em Teste']
